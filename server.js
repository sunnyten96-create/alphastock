...
const historyStartDate = "1999-01-01";
...
async function fetchDailyFromYahoo(symbol) {
  ...
  const period1 = Math.floor(new Date(`${historyStartDate}T00:00:00Z`).getTime() / 1000);
  const period2 = Math.floor(Date.now() / 1000);
  const url = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(yahoo)}?period1=${period1}&period2=${period2}&interval=1d&events=history`;
