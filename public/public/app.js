...
const defensive = new Set(["GLD", "TLT", "IEF", "SHY", "BIL", "UUP", "FXE"]);
...
async function fetchAsset(symbol) {
  const res = await fetch(`/api/daily?symbol=${encodeURIComponent(symbol)}`);
...
async function fetchChartRows(symbol, frame = chartState.frame) {
  const res = await fetch(`/api/chart?symbol=${encodeURIComponent(symbol)}&frame=${encodeURIComponent(frame)}`);
  const payload = await res.json();
...
async function fetchNews(symbol) {
  const res = await fetch(`/api/news?symbols=${encodeURIComponent(symbol)}`);
  if (!res.ok) return [];
  const payload = await res.json();
...
async function previewKisOrders() {
  const res = await fetch("/api/kis/orders/preview", {
    method: "POST",
    headers: { "content-type": "application/json" },
...
function createKisOrdersFromPlan() {
  ...
  const minOrderValue = Math.max(50, plan.total * 0.006);
...
async function submitKisOrders() {
  ...
  if (!confirm) return;
  const res = await fetch("/api/kis/orders", {
    method: "POST",
    headers: { "content-type": "application/json" },
...
function retAt(rows, i, days) {
  return i >= days && rows[i - days] ? rows[i].close / rows[i - days].close - 1 : 0;
}
...
function marketState(spy, i) {
  const { rows, ind } = spy;
  const vix = spy.vix?.rows?.[i]?.close;
  const tnx = spy.tnx?.rows?.[i]?.close;
  const tnxRise = spy.tnx?.rows?.[i - 63] ? (tnx - spy.tnx.rows[i - 63].close) : 0;
  let heat = 0;
...
function assetScore(asset, spy, i, flavor) {
  ...
  const rel = r3 - retAt(spy.rows, i, 63);
...
function adaptiveLeverageState(assets, spy, i) {
  ...
  const qi = Math.min(i, qqq.rows.length - 1);
...
function weightsForAdaptiveLeverage(assets, spy, i) {
  ...
  const safe = map.BIL ? "BIL" : map.SHY ? "SHY" : "GLD";
  if (state.riskBudget <= 0.08) {
    const weights = {};
    if (map[safe]) weights[safe] = 0.70;
    if (map.GLD) weights.GLD = 0.20;
    if (map.TLT) weights.TLT = 0.10;
  ...
  const ranked = pool
    .map((symbol) => {
      ...
      if (!asset || !asset.rows[i] || i < 252) return null;
      return { symbol, score: adaptiveLeverageScore(asset, qqq, i, qi), vol: realizedVol(asset.rows, i, 63), defensive: false };
    })
    .filter(Boolean)
  ...
  if (used < 0.98) {
    ...
    if (map[filler]) weights[filler] = (weights[filler] || 0) + (1 - used);
  }
...
function weightsForStrategy(assets, spy, i, flavor) {
  ...
  const rows = assets.map((asset) => ({
    ...
    vol: realizedVol(asset.rows, i, 63),
    defensive: defensive.has(asset.symbol)
  })).filter((x) => x.score >= (x.defensive ? 46 : 58));
...
function researchScore(asset, spy, i, config = {}) {
  ...
  const spy3 = retAt(spy.rows, Math.min(i, spy.rows.length - 1), 63);
...
function weightsForAutonomousResearch(assets, spy, i, research = activeResearchJson()) {
  ...
  const safe = assets.find((asset) => asset.symbol === "BIL") ? "BIL" : assets.find((asset) => asset.symbol === "SHY") ? "SHY" : "GLD";
  const ranked = assets
    .filter((asset) => asset?.rows?.[i] && i >= 252)
    .map((asset) => {
  ...
  if (safeRoom > 0.0001 && safe) {
    ...
    if (uupWeight) weights.UUP = (weights.UUP || 0) + uupWeight;
    const gldWeight = assets.find((asset) => asset.symbol === "GLD") && state.heat < 0.52 ? Math.min(0.18, safeRoom * 0.18) : 0;
...
function monthReturn(weights, assetMap, i, nextI) {
  ...
  Object.entries(weights).forEach(([symbol, w]) => {
    const rows = assetMap[symbol]?.rows;
    if (rows?.[i] && rows?.[nextI]) out += w * (rows[nextI].close / rows[i].close - 1);
  });
  return out;
}
...
function dailyReturn(weights, assetMap, i) {
  ...
  Object.entries(weights).forEach(([symbol, w]) => {
    const rows = assetMap[symbol]?.rows;
    if (rows?.[i] && rows?.[i - 1]) out += w * (rows[i].close / rows[i - 1].close - 1);
  });
  return out;
}
...
function defensiveWeights(assetMap) {
  const weights = {};
  if (assetMap.BIL) weights.BIL = 0.70;
  else if (assetMap.SHY) weights.SHY = 0.70;
  if (assetMap.GLD) weights.GLD = 0.20;
  if (assetMap.TLT) weights.TLT = 0.10;
  const used = Object.values(weights).reduce((sum, weight) => sum + weight, 0);
  if (used < 0.99 && assetMap.SHY) weights.SHY = (weights.SHY || 0) + (1 - used);
  return normalizeWeights(weights);
}
...
function recoveryReentryWeights(assetMap) {
  const weights = {};
  ...
  const safe = assetMap.BIL ? "BIL" : assetMap.SHY ? "SHY" : "GLD";
  if (assetMap[safe]) weights[safe] = Math.max(0, 1 - Object.values(weights).reduce((sum, weight) => sum + weight, 0));
  return normalizeWeights(weights);
}
...
function applyWeeklyRiskGuard(pack, assetMap, assets, spy, i) {
  ...
  const soxlI = soxl ? Math.min(i, soxl.rows.length - 1) : -1;
  if (soxl && soxlI >= 21) {
  ...
  if (smh && soxx && qi >= 10 && i >= 10) {
    const semiWeak =
      retAt(smh.rows, i, 10) - retAt(qqq.rows, qi, 10) < -0.04 &&
      retAt(soxx.rows, i, 10) - retAt(qqq.rows, qi, 10) < -0.04;
    if (semiWeak) {
...
function monthReturnWithWeeklyGuard(pack, assetMap, assets, spy, i, nextI, benchmark) {
  ...
  for (let day = i + 1; day <= nextI; day += 1) {
    ...
    if (!defenseMode && lossCut > activeCut) {
      weights = reduceRisk(pack.weights, lossCut, assetMap.BIL ? "BIL" : assetMap.SHY ? "SHY" : "GLD");
      activeCut = lossCut;
      guards.push({
    ...
    if ((day - i - 1) % 5 !== 0) {
      ...
      if (benchmark?.rows?.[day] && benchmark?.rows?.[day - 1]) {
        benchmarkEquity *= benchmark.rows[day].close / benchmark.rows[day - 1].close;
      } else {
        benchmarkEquity *= spy.rows[day].close / spy.rows[day - 1].close;
      }
      continue;
    }
    ...
    if (guarded.guard?.level === "defense" && !defenseMode) {
      ...
      defenseMode = true;
      activeCut = 1;
      guards.push({ date: spy.rows[scanIndex].date, ...guarded.guard });
    ...
    if (reentry.ok && riskExposure(weights) < 0.35) {
      weights = recoveryReentryWeights(assetMap);
      defenseMode = false;
...
function evaluateStrategy(assetMap, assets, spy, flavor, fromI, toI, benchmarkAsset = spy) {
  ...
  for (let i = fromI; i + 21 <= toI; i += 21) {
    ...
    benchmarkReturns.push(benchmarkAsset.rows[i + 21].close / benchmarkAsset.rows[i].close - 1);
  }
...
function applyBenchmarkCore(pack, assetMap, benchmark, spy, i) {
  if (!benchmark?.rows?.[i] || defensive.has(benchmark.symbol)) return pack;
  if (pack.components.some((item) => item.key === "adaptiveLeverage")) return pack;
  const state = marketState(spy, i);
  const benchmarkTrend = benchmark.rows[i].close > benchmark.ind.sma200[i] && retAt(benchmark.rows, i, 63) > 0;
...
function actionText(delta, total) {
  if (delta > total * 0.006) return "매수";
  if (delta < -total * 0.006) return "매도";
  return "유지";
}
...
function buildContributionPlan(assetMap, result) {
  ...
  const rows = symbols.map((symbol) => {
    ...
    const action = delta > total * 0.006 ? "추가매수" : delta < -total * 0.006 ? "일부매도" : "유지";
...
function buildDcaReplay(result, initialAmount, monthlyContribution, fromIndex = 0, toIndex = result.events.length - 1) {
  ...
  for (let idx = Math.max(0, fromIndex); idx <= Math.min(result.events.length - 1, toIndex); idx += 1) {
    ...
    portfolioValue = (portfolioValue + monthlyContribution) * (1 + event.nextReturn);
    benchmarkValue = (benchmarkValue + monthlyContribution) * (1 + event.spyReturn);
    points.push({
...
function simulateWalkForward(assetMap, benchmarkSymbol = "SPY", research = activeResearchJson()) {
  ...
  for (const i of monthlyRebalanceIndices(spy.rows, 504)) {
    const state = marketState(spy, i);
    ...
    const guardedReturn = monthReturnWithWeeklyGuard(pack, assetMap, assets, spy, i, nextI, benchmark);
...
function monthStartCapital(result, index, initialAmount, monthlyContribution) {
  ...
  return {
    ...
    portfolioValue: portfolioValue + monthlyContribution,
    benchmarkValue: benchmarkValue + monthlyContribution
  };
}
...
function renderReplayTable(result, startIndex = 0, endIndex = result.events.length - 1) {
  ...
  table.innerHTML = result.events.slice(from, to + 1).map((event, offset) => {
    ...
    const guardTrail = event.guards?.length ? `<br><span class="muted">${event.guards.map((g) => `${g.date} ${g.label}`).join(" / ")}</span>` : "";
...
function renderAll(assetMap, result) {
  ...
  $("#portfolio-summary").innerHTML = `
    ...
    <div><span class="label">감시 발동</span><strong>${result.guardCount || 0}회</strong></div>`;
